"""Tests for trendbot.engine.validation.

The module exists to argue that a backtest result is fake, so the tests here are
mostly about it refusing to be reassuring: the deflation term must actually deflate,
the number of configurations must be impossible to leave blank, and the synthetic
data must contain a real overnight gap so that a mis-timed fill cannot hide.

Everything is offline. Price panels come from ``synthetic_prices``, which is a
seeded generator.
"""
...
def test_synthetic_prices_differs_across_seeds():
    ...
    # not merely a shifted copy: the paths diverge everywhere after bar 0
...
def test_synthetic_prices_has_a_real_overnight_gap_between_close_and_next_open():
    # An engine that fills at the wrong bar is only detectable if open != prior
    # close. Every bar after the first must gap.
    ...
    # gap sd is overnight_gap_fraction * daily sd = 0.4 * 0.16/sqrt(252) = 0.00403;
    # observed 0.00405 on this machine.
...
def test_synthetic_prices_realises_the_requested_volatility_and_zero_drift():
    ...
    # observed 0.1596 on this machine
    assert 0.150 < ann_vol < 0.172
    # driftless: the mean annualised log return across 12 paths sits near zero
...
def test_psr_is_exactly_one_half_when_observed_equals_the_benchmark():
    ...
    # and with non-normal moments too - the benchmark comparison is what drives it
...
def test_psr_is_near_one_for_a_strong_sharpe_over_many_observations():
    ...
    # annualised Sharpe of this sample is 1.099 on this machine
...
def test_psr_refuses_higher_moments_that_make_the_estimator_undefined():
    # 1 - skew*sr + 0.25*(kurt-1)*sr^2 = 1 - 5 + 0 = -4
...
def test_expected_max_sharpe_is_exactly_zero_for_a_single_configuration():
    # The closed form contains Z^-1(1 - 1/N) = Z^-1(0) at N=1. There is nothing to
    # deflate with one trial, so the documented handling is an exact 0.0.
...
def test_expected_max_sharpe_increases_with_the_number_of_configurations():
    ...
    assert all(b > a for a, b in zip(values, values[1:])), values
    # observed: N=2 -> 0.0520, N=100 -> 0.2531 per-period at variance 0.01
...
def test_expected_max_sharpe_increases_with_the_trial_variance():
    ...
    assert values[0] == 0.0
    assert all(b > a for a, b in zip(values, values[1:])), values
    # it scales as the standard deviation of the trial Sharpes
...
# --------------------------------------------------------------------------------------
# deflated Sharpe: the API contract
# --------------------------------------------------------------------------------------


def test_deflated_sharpe_requires_the_configuration_count_as_a_positional_argument():
    # BUILD_PROMPT step 5: "Configurations tried is a required argument with no
    # default." A default here would be a lie with a number attached.
    r = _returns(1000)
...
def test_walk_forward_produces_the_expected_number_of_windows():
    r = _returns(2016, seed=1)  # exactly 8 years at 252 days
    wf = v.walk_forward(r, train_years=3.0, test_years=1.0, step_years=1.0)
    # windows start at 0, 252, 504, 756, 1008; the last needs 1008+1008 = 2016 rows
...
def test_walk_forward_test_period_starts_on_the_bar_immediately_after_training_ends():
    ...
    for _, row in wf.windows.iterrows():
        ...
        assert j == i + 1  # no gap and no overlap: the live period is contiguous
...
def test_walk_forward_raises_when_the_series_cannot_hold_one_window():
    ...
    # exactly one window's worth is enough, one bar fewer is not
...
# --------------------------------------------------------------------------------------
# noise test - the fast leg of section 7 step 1
# --------------------------------------------------------------------------------------


def test_the_engine_earns_nothing_on_driftless_synthetic_data(cfg):
    result = v.noise_test(cfg, seeds=(0, 1), n_days=1500)
    # observed on this machine: mean strategy Sharpe -0.094, buy-and-hold -0.155
...
def test_noise_test_result_reports_a_readable_summary(cfg):
    ...
    assert math.isnan(result.std_sharpe)  # one seed: no cross-seed dispersion


# --------------------------------------------------------------------------------------
# BUILD_PROMPT step 5 gate
# --------------------------------------------------------------------------------------
...
def test_step_5_gate_mining_noise_manufactures_a_sharpe_that_deflation_rejects(cfg):
    ...
    "Sweeping 100 parameter combinations on pure noise produces a best in-sample
    Sharpe above 1.0 while the deflated Sharpe is not significant."

    Observed on this machine (6 seeds x 100 configurations x 750 days): best mined
    Sharpe +1.805, three of six datasets above 1.0, none significant; seed 2 has
    PSR(0) = 0.9991 against a deflated Sharpe of 0.6897.
    """
    ...
    # leg 1: mining pure noise manufactures a Sharpe above 1.0
    assert study.n_mined_above_one >= 1, study
    assert study.max_best_sharpe > 1.0, study

    # leg 2: the deflated Sharpe calls none of them significant
    assert study.any_significant is False, study.table.to_string()

    # the pointed fact: at least one dataset would have fooled a naive PSR
    table = study.table
    ...
    # deflation is doing real work, not shaving a rounding error off
    assert float((table["psr_vs_zero"] - table["deflated_sharpe"]).max()) > 0.2

    # the deflation term is a positive benchmark on every dataset, and the mined
    # configurations are not all the same backtest
...
def test_a_single_noise_sweep_mines_a_positive_sharpe_out_of_nothing(cfg):
    ...
    # a hundred tries on driftless data still finds something that looks like an edge
