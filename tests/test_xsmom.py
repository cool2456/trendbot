"""STEP 2's GATE — the selected set matches a hand-built fixture with known ranks.

PREREG_002.md section 3::
...
The fixtures here are small enough that the right answer is arithmetic done by hand
in the test, not a re-implementation of the rule. Everything asserted is a number a
...
# --------------------------------------------------------------------------------------
# the hand-built fixture: six names, three buckets, ranks known by construction
# --------------------------------------------------------------------------------------

# formation 4 bars, skip 1 bar. On row 4 the rule reads P(row 3) / P(row 0) - 1.
# Every name starts at 100 on row 0, so the momentum IS the row-3 price over 100 - 1.
# Every momentum is negative, which is not a special case for a long-only *relative*
# rule: section 3 ranks instruments against each other and never asks whether the
# winner rose. Choosing the fixture this way also makes the "excluded, not zeroed"
# counterfactual below bite — a name silently given momentum 0 would rank FIRST.
...
# descending: F -0.02, A -0.05, B -0.10, C -0.15, D -0.20, E -0.25
HAND_EXPECTED_ORDER = ["F", "A", "B", "C", "D", "E"]
# six names, three buckets -> 6 // 3 == 2 each
...
def hand_prices() -> pd.DataFrame:
    ...
    frame.iloc[0] = 100.0
    frame.iloc[1] = 999.0  # never read by the rule at row 4; a deliberate distractor
    frame.iloc[2] = 888.0  # ditto
    frame.iloc[3] = [HAND_ROW3[n] for n in HAND_NAMES]
    frame.iloc[4] = 777.0  # the *current* bar, which the 21-day skip must not read
    frame.iloc[5] = 666.0
    return frame
...
def test_momentum_is_the_ratio_the_document_states(hand_prices):
    ...
    # rows without a full formation window have no momentum at all
    assert momentum.iloc[:4].isna().all().all()


def test_the_skip_means_the_current_bar_is_not_read(hand_prices):
    """Row 4's price is 777 for everyone; if it leaked in, every momentum would tie."""
    momentum = cross_sectional_momentum(hand_prices, formation_days=4, skip_days=1)
...
def test_an_instrument_without_history_is_excluded_not_zeroed(hand_prices):
    """The distinction section 3 draws, made decisive.

    ``G`` has no history. Every real name's momentum is negative, so a ``G`` silently
    given momentum 0 would rank FIRST of seven and be bought at half the book — an
    instrument that has never traded, held as the strongest momentum name in the
    universe. That is the failure section 3's "excluded from that ranking" prevents,
    and the counterfactual is asserted below so the test fails if the distinction ever
    stops mattering.
    """
    ...
    # ... and the counterfactual the document rules out.
...
def test_ties_are_broken_deterministically_by_column_order():
    ...
    frame.iloc[3] = 110.0  # every name has exactly the same momentum
...
# --------------------------------------------------------------------------------------
# bucket sizing — the convention for a count that does not divide
# --------------------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    "n_valid,expected",
    [
        (41, (8, 8, 8, 8, 9)),  # the pre-registered universe: "highest ~8 of 41"
        (40, (8, 8, 8, 8, 8)),
        (44, (8, 8, 8, 8, 12)),
        (10, (2, 2, 2, 2, 2)),
        (5, (1, 1, 1, 1, 1)),
        (4, (0, 0, 0, 0, 0)),  # fewer names than buckets: no ranking exists
        (0, (0, 0, 0, 0, 0)),
    ],
)
...
def test_the_top_bucket_holds_the_eight_names_section_3_names(cfg002):
    """41 instruments, top quintile of 8, each at 1/8 — checked on the real universe."""
    ...
    # before any instrument has a full formation window the book is entirely cash
    assert np.allclose(weights[~ranked].to_numpy(), 0.0)


def test_the_traded_set_is_exactly_q1(cfg002):
    """Section 3's position and section 8's Q1 must be the same set, not merely similar."""
